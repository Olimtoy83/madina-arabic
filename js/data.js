...
].map((lesson) => ({ ...lesson, words: lesson.words.map(([id, arabic, ru, uz, letters, hasAudio = true, imageSrc]) => { const path = `assets/audio/words/word-${String(id).padStart(3, "0")}.mp3`; return { id, arabic, translations: { ru, uz }, letters, audio: hasAudio ? { src: path, expectedPath: path } : {}, image: imageSrc ? { src: imageSrc } : {} }; }), understand: Array.isArray(lesson.understand) ? lesson.understand.map((item) => ({ ...item, questionAudio: { expectedPath: `assets/audio/understand/03A/${item.id}-question.mp3` } })) : lesson.understand }));
